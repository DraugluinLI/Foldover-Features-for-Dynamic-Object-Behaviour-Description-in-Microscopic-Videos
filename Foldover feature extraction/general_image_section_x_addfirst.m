%% The purpose of this file is to slice the X-axis direction (that is, the column direction of the matrix) for the general image, first add each slice and then divide it into 224*224.
clc;
clear;
close all;
flag=0;
...
for i=1:25
    ...
    if flag==1
        ...
        namelist = dir(strcat(path1,'x\',num2str(i),'\*.mat'));
        len = length(namelist);
        max_num=0;
        num_flag=0;
        for i1 = 1:len        
        ...
        for i2 = 1:len   
            ...
            s33=char(s22(1));
            s33=str2num(s33);
            load(strcat(path1,'\x\',num2str(i),'\',file_name1));
        ...
        save([path1,'\x_addfirst\',num2str(i),'.mat'],'sum');%Save mat file
        imwrite(sum,strcat(path1,'pic\x_addfirst\',num2str(i),'.jpg'));
    end
    flag=0;
end